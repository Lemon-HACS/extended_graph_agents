...
- **조건 노드**: LLM 없이 HA 엔티티 상태 등 Jinja2 템플릿으로 결정적(deterministic) 라우팅
...
| 노드 타입 | 아이콘 | 용도 |
|-----------|--------|------|
| **라우터** | 🔀 | 사용자 입력을 분석해 다음 경로 결정 (LLM 사용) |
| **조건** | 🔁 | HA 엔티티 상태 등 Jinja2 조건으로 결정적 라우팅 (LLM 미사용) |
...
**LLM 호출 없이** Jinja2 템플릿으로 조건을 평가하여 상태 변수를 세팅합니다.
HA 엔티티 상태, 수치 비교, 시간대 등 어떤 조건도 사용할 수 있습니다.

`conditions` 리스트를 **순서대로** 평가하며, 처음으로 truthy한 항목의 `value`가 선택됩니다.
모두 해당하지 않으면 `default` 값이 사용됩니다.

```yaml
- id: check_presence
  type: condition
  output_key: route          # 결과가 저장될 상태 변수 이름
  conditions:
    - when: "{{ is_state('person.john', 'home') }}"
      value: "home"
    - when: "{{ states('sensor.temperature') | float > 28 }}"
      value: "too_hot"
    - when: "{{ now().hour < 8 or now().hour >= 22 }}"
      value: "night"
  default: "away"            # 모든 조건 미일치 시 폴백 (선택사항)
```

이후 에지 조건으로 조건 노드 결과에 따라 분기합니다:

```yaml
edges:
  - source: check_presence
    target: welcome_node
    condition:
      variable: route
      value: "home"
  - source: check_presence
    ...
    condition:
      variable: route
      value: "too_hot"
  - source: check_presence   # unconditional = 어떤 조건도 매치되지 않을 때 폴백
    target: default_node
```

> **Tip**: HA 템플릿에서 사용 가능한 모든 함수(`states()`, `is_state()`, `state_attr()`, `now()` 등)를 `when` 템플릿에서 그대로 사용할 수 있습니다.
...
### v1.0.49
- **조건 노드(`condition`) 추가**: LLM 없이 HA 엔티티 상태 등 Jinja2 템플릿으로 결정적 라우팅
  - `conditions` 리스트를 순서대로 평가, 첫 truthy 항목의 `value`로 상태 변수 세팅
  - `default` 폴백 값 지원
  - HA 템플릿 함수(`states()`, `is_state()`, `state_attr()`, `now()` 등) 모두 사용 가능
  - 기존 에지 조건(`condition.variable/value`)과 조합하여 완전한 LLM-free 분기 구성 가능
